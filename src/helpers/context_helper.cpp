...
bool ContextHelper::is_interface_enabled;
...
std::chrono::high_resolution_clock::time_point ContextHelper::tic_toc_timer;

void ContextHelper::tic() {
	tic_toc_timer = std::chrono::high_resolution_clock::now();
}

uint32_t ContextHelper::toc(bool reset,bool pr) {
	auto g_end = std::chrono::high_resolution_clock::now();
	static long long coef = 0;
	coef += 1.0f;
	static long long avg_total_duration = 0;
	...
	long long duration = std::chrono::duration_cast<std::chrono::microseconds>(g_end - tic_toc_timer).count();

	avg_total_duration = (avg_total_duration * (coef - 1) + duration) / coef;
	if (pr)
	std::cout << "Elapsed time: " << avg_total_duration << " microseconds" << std::endl;

	return (uint32_t)(avg_total_duration);
}
...
void ContextHelper::print_opengl_info()
{
	...
	std::cout << "    Maximum Uniform Buffer Object (UBO) size: "
		<< maxUBOSize << " Bytes" << std::endl;
	GLint maxSSBOSize = 0;
	glGetIntegerv(GL_MAX_SHADER_STORAGE_BLOCK_SIZE, &maxSSBOSize);

	std::cout << "    Maximum Shader Storage Buffer Object (SSBO) size: "
		<< maxSSBOSize << " Bytes" << std::endl;
	...
	std::cout << "    Maximum shared memory available in workgroup: " << max_smem_bytes << " Bytes" << std::endl ;
	...
	std::cout << "    Maximum workgroup counts (for dispatchCompute command): x=" << maxX << " y=" << maxY << " z=" << maxZ << std::endl << std::endl;
}
...
void ContextHelper::begin_frame()
{
	...
	time_frame_s = glm::clamp(std::chrono::duration_cast<std::chrono::microseconds>(current_time - previous_time).count() * 1e-6f, 0.0f, 1.0f);
	...
	int key_state_space = glfwGetKey(window, GLFW_KEY_SPACE);
	static int key_state_previous_space = GLFW_RELEASE;
	if (key_state_space == GLFW_PRESS && key_state_previous_space == GLFW_RELEASE)
		is_interface_enabled = !is_interface_enabled;
	key_state_previous_space = key_state_space;
...
void ContextHelper::end_frame()
{
	...
	//if pressing P takse a screenshot
	static int key_state_previous_p = GLFW_RELEASE;
	if (glfwGetKey(window, GLFW_KEY_P) == GLFW_PRESS)
	{
		if (key_state_previous_p == GLFW_RELEASE)
			save_screenshot();
		key_state_previous_p = GLFW_PRESS;
	}
	else
		key_state_previous_p = GLFW_RELEASE;
...
void ContextHelper::reset_prog_timer()
{
	m_start_time = std::chrono::high_resolution_clock::now();
...
void ContextHelper::save_screenshot() 
{
	std::string subfolder = ROOT_FOLDER + std::string("captures");
	std::filesystem::create_directories(subfolder); // ensures folder exists

	int idx = 0;
	std::string fname;
	do {
		std::ostringstream ss;
		ss << subfolder<<"/screenshot_"
	...
	int width = ContextHelper::resolution.x;
	int height = ContextHelper::resolution.y;
	...
	for (int y = 0; y < height / 2; y++) {
		...
		for (int x = 0; x < width * 4; x++) {
			std::swap(pixels[y * width * 4 + x], pixels[opp * width * 4 + x]);
		}
	}
	
	stbi_write_png(fname.c_str(), width, height, 4, pixels, width * 4);
