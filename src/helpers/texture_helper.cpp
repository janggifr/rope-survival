...
void Texture2D::write_ptr_to_gpu(const void* ptr)
{
	glTextureSubImage2D(m_tex_id, 0, 0, 0, m_size.x, m_size.y, m_format.cpu_channels, m_format.cpu_type, ptr);
	if (m_mipmap_auto && m_levels_count > 1) // compute mipmaps
	{
...
void Texture2DArray::set_format_params(const CpuGpuTransfertFormat format)
{
	m_format = format;
}

void Texture2DArray::set_filtering_params(const GLenum mag_filter, GLenum min_filter, const float max_anisotropy, const GLenum wrap_s, const GLenum wrap_t)
{
	m_mag_filter = mag_filter;
	m_min_filter = min_filter;
	m_max_anisotropy = max_anisotropy;
...
void Texture2DArray::create_empty(const glm::uvec3 size)
{
	m_size = size;
	glTextureStorage3D(m_tex_id, 1, m_format.gpu_format, m_size.x, m_size.y, m_size.z);
}

void Texture2DArray::re_create_empty(const glm::uvec3 size)
{
	glDeleteTextures(1, &m_tex_id);
	glCreateTextures(GL_TEXTURE_2D, 1, &m_tex_id);
	m_size = size;
...
void Texture2DArray::set_slot(const GLuint slot)
{
	m_slot = slot;
	glBindTextureUnit(slot, m_tex_id);
}

void Texture2DArray::bind_to_image(GLenum access_flag, int slot)
{
