...
public:
	...
	void write_ptr_to_gpu(const void* ptr);
...
public:
	Texture2DArray();
	~Texture2DArray();
	void set_format_params(const CpuGpuTransfertFormat format);
	void set_filtering_params(const GLenum mag_filter = GL_LINEAR, GLenum min_filter = GL_LINEAR, const float max_anisotropy = 16.0f, const GLenum wrap_s = GL_REPEAT, const GLenum wrap_t = GL_REPEAT);
	void create_empty(const glm::uvec3 size);
